...
const queryAgentAll = require('../../db_operations/agent_db_ops').queryAgentAll;
const queryAgentById = require('../../db_operations/agent_db_ops').queryAgentById;
...
.get((req, res, next)=>{
    if(has_value(req.user) && has_value(req.user._id)){
        res.statusCode = 200;
        queryAgentAll(req.user._id, (err, result)=>{
            if(err){
                res.statusCode = 403;
...
.get((req, res, next)=>{
    if(has_value(req.user) && has_value(req.user._id) && has_value(req.params["id"])){
        res.statusCode = 200;
        queryAgentById(req.user._id, req.params['id'], (err, result)=>{
            if(err){
                res.statusCode = 403;
                res.json({success: false, reasons:[], value: result});
            }else{
