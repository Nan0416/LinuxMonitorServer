...
verifyPrivilegeRouter.route("/:privilege")
.post((req, res, next)=>{
    if(has_value(req.params.privilege) && has_value(req.body.key)){
        verifyPrivilege(req.params.privilege, req.body.key, (err, result)=>{
            if(err){
                res.statusCode = 400;
